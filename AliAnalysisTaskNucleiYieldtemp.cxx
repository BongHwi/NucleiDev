...
///\cond CLASSIMP
ClassImp(AliAnalysisTaskNucleiYieldtemp);
///\endcond
...
bool AliAnalysisTaskNucleiYieldtemp::IsInTRD(float pt, float phi, float sign) {
    bool withTRD[2]{
...
///
AliAnalysisTaskNucleiYieldtemp::AliAnalysisTaskNucleiYieldtemp(TString taskname)
  :AliAnalysisTaskSE(taskname.Data())
   ...
   ,fTPCcounts{nullptr}
   ,fTPCcountsQA{nullptr}
   ,fTPCcountsQA2{nullptr}
   ,fTPCcountsQA3{nullptr}
   ,fTPCcountsQA4{nullptr}
   ,fTPCcountsQA5{nullptr}
   ,fTPCcountsQA6{nullptr}
   ,fTPCcountsQA7{nullptr}
...
/// Standard destructor
///
AliAnalysisTaskNucleiYieldtemp::~AliAnalysisTaskNucleiYieldtemp(){
...
/// \return void
///
void AliAnalysisTaskNucleiYieldtemp::UserCreateOutputObjects() {
  ...
  if (fIsMC) {
    ...
    for (int iC = 0; iC < 2; ++iC) {
      ...
      fTPCcountsQA[iC] = new TH1F(Form("f%cTPCcountsQA",letter[iC]),"; n_{#sigma} d",fSigmaNbins,sigmaBins);
    ...
    fTPCcountsQA2 = new TH1F("fTPCcountsQA","; n_{#sigma} d",200,-10,10);
    fTPCcountsQA3 = new TH1F("fTPCcountsQA_after","; n_{#sigma} d",200,-10,10);
    fTPCcountsQA4 = new TH1F("fTPCcountsQA_check1","; n_{#sigma} d",200,-10,10);
    fTPCcountsQA5 = new TH1F("fTPCcountsQA_check1_after","; n_{#sigma} d",200,-10,10);
    fTPCcountsQA6 = new TH2F("fTPCcountsQA_check2",";#it{p}_{T} (GeV/#it{c}); n_{#sigma} d",100,0,10,200,-10,10);
    fTPCcountsQA7 = new TH2F("fTPCcountsQA_check2_after",";#it{p}_{T} (GeV/#it{c}); n_{#sigma} d",100,0,10,200,-10,10);
    fList->Add(fTPCcountsQA2);
    fList->Add(fTPCcountsQA3);
    fList->Add(fTPCcountsQA4);
    fList->Add(fTPCcountsQA5);
    fList->Add(fTPCcountsQA6);
    fList->Add(fTPCcountsQA7);
  }
...
/// \return void
///
void AliAnalysisTaskNucleiYieldtemp::UserExec(Option_t *){
  ...
  if (fParticle == AliPID::kUnknown) {
    ::Error("AliAnalysisTaskNucleiYieldtemp::UserExec", "No particle type set");
    PostData(1, fList);
    if(fSaveTrees){
      PostData(2, fRTree);
      if(fIsMC){
  ...
  if (fIsMC || fPtShape) {
    ...
    if (!stack)
      ::Fatal("AliAnalysisTaskNucleiYieldtemp::UserExec","MC analysis (or pt shape) requested on a sample without the MC particle array.");
...
/// \return void
///
void AliAnalysisTaskNucleiYieldtemp::Terminate(Option_t *) {
  return;
}
...
/// \return void
///
void AliAnalysisTaskNucleiYieldtemp::SetCentBins(Int_t nbins, Float_t *bins) {
...
/// \return void
///
void AliAnalysisTaskNucleiYieldtemp::SetDCABins(Int_t nbins, Float_t min, Float_t max) {
...
/// \return void
///
void AliAnalysisTaskNucleiYieldtemp::SetDCABins(Int_t nbins, Float_t *bins) {
...
/// \return void
///
void AliAnalysisTaskNucleiYieldtemp::SetPtBins(Int_t nbins, Float_t min, Float_t max) {
  const float delta = (max - min) / nbins;
  fPtBins.Set(nbins + 1);
  for (int iB = 0; iB < nbins; ++iB) {
...
/// \return void
///
void AliAnalysisTaskNucleiYieldtemp::SetPtBins(Int_t nbins, Float_t *bins) {
...
/// \return void
///
void AliAnalysisTaskNucleiYieldtemp::SetCustomTPCpid(Float_t *par, Float_t sigma) {
  if (par == 0x0 && sigma <= 0) {
    fCustomTPCpid.Set(1);
  } else {
    fCustomTPCpid.Set(6);
...
float AliAnalysisTaskNucleiYieldtemp::GetTPCsigmas(AliVTrack* t) {
...
float AliAnalysisTaskNucleiYieldtemp::GetTOFsigmas(AliVTrack* t) {
  AliNanoAODTrack* nanoT = dynamic_cast<AliNanoAODTrack*>(t);
...
///
int AliAnalysisTaskNucleiYieldtemp::PassesPIDSelection(AliAODTrack *t) {
...
int AliAnalysisTaskNucleiYieldtemp::PassesPIDSelection(AliNanoAODTrack *t) {
  bool tofPID = true, itsPID = true, tpcPID = true, electronRejection = true;
...
/// \return void
///
void AliAnalysisTaskNucleiYieldtemp::SetTOFBins(Int_t nbins, Float_t min, Float_t max) {
  fTOFnBins = nbins;
  fTOFlowBoundary = min;
...
/// \return void
///
void AliAnalysisTaskNucleiYieldtemp::SetDCAzBins(Int_t nbins, Float_t limit) {
  fDCAzNbins = nbins;
  fDCAzLimit = limit;
}
...
/// \return void
///
void AliAnalysisTaskNucleiYieldtemp::SetSigmaBins(Int_t nbins, Float_t limit) {
  fSigmaNbins = nbins;
  fSigmaLimit = limit;
}
...
/// \return void
///
void AliAnalysisTaskNucleiYieldtemp::SetTOFSigmaBins(Int_t nbins, Float_t limit) {
...
/// \return void
///
void AliAnalysisTaskNucleiYieldtemp::SetParticleType(AliPID::EParticleType part) {
  fParticle = part;
...
///
Bool_t AliAnalysisTaskNucleiYieldtemp::Flatten(float cent) {
...
/// \return void
///
void AliAnalysisTaskNucleiYieldtemp::PtCorrection(float &pt, bool positiveCharge) {
...
///
int AliAnalysisTaskNucleiYieldtemp::GetNumberOfITSclustersPerLayer(AliVTrack *track, int &nSPD, int &nSDD, int &nSSD) {
  if (!track) return -1;
  nSPD = 0u;
  nSDD = 0u;
  nSSD = 0u;
...
void AliAnalysisTaskNucleiYieldtemp::SetSLightNucleustemp(AliAODMCParticle* part, SLightNucleustemp& snucl) {
  ...
  if (part->IsPhysicalPrimary())
    snucl.flag = SLightNucleustemp::kPrimary;
  else if (part->IsSecondaryFromWeakDecay())
    snucl.flag = SLightNucleustemp::kSecondaryWeakDecay;
  else
    snucl.flag = SLightNucleustemp::kSecondaryMaterial;
}
...
///
float AliAnalysisTaskNucleiYieldtemp::HasTOF(AliVTrack *track, AliPIDResponse *pid) {
...
/// \return \f$\beta\f$ of the particle, -1 means that there is no correct prolongation in TOF.
///
float AliAnalysisTaskNucleiYieldtemp::HasTOF(AliNanoAODTrack *track, AliPIDResponse *pid) {
  const float len = track->GetIntegratedLength();
...
///
Bool_t AliAnalysisTaskNucleiYieldtemp::IsSelectedTPCGeoCut(AliAODTrack *track) {
...
Bool_t AliAnalysisTaskNucleiYieldtemp::IsSelectedTPCGeoCut(AliNanoAODTrack *track) {
